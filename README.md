...
### 13.1 网关访问 AI 接口返回 500

- 检查 Nacos 是否已注册 `ai-svc`
- 检查 `gateway-svc` 路由是否指向 `lb://ai-svc`
- 检查 AI 服务端口 `8086` 是否启动

### 13.2 交易下单报事务回滚

- 检查服务层是否在 `@Transactional` 内吞异常
- 优先在控制层做兜底，服务层保持异常透传保证回滚一致性
...
如果你计划把它发布为公开仓库，建议同时补充：

- `LICENSE`
- `CONTRIBUTING.md`
- `SECURITY.md`
- GitHub Actions CI（编译、测试、静态检查）
